...
describe('database migration runner', () => {
  ...
  it('retains the exact migration 26 ledger entry in the pre-push fallback', async () => {
    const pushMigration = databaseMigrations().find((candidate) => candidate.version === 26);
    if (!pushMigration) throw new Error('The migration-26 rollback boundary is missing.');

    expect(pushMigration.name).toBe('add private ios push installations and transactional social alerts');
    expect(migrationChecksum(pushMigration))
      .toBe('f10eccaeee9b15deed76a97c746933697b3a9761b2a702557e8af5fa1064d9ee');

    const database = fakeDatabase({
      applied: [{
        version: pushMigration.version,
        name: pushMigration.name,
        checksum: migrationChecksum(pushMigration),
      }],
    });

    await expect(runDatabaseMigrations(database.pool, { migrations: [pushMigration] }))
      .resolves.toEqual({ applied: [], currentVersion: 26 });
  });
