...
describe('Reaction Test scoring', () => {
  ...
  it('shows two decimal places without changing stored timing precision', () => {
    expect(formatReactionTime(380)).toBe('0.38');
    expect(formatReactionTime(381)).toBe('0.38');
    expect(formatReactionTime(30)).toBe('0.03');
    expect(formatReactionTime(999.6)).toBe('1.00');
...
it('places every random draw after actual voice completion with unchanged tone offsets', () => {
  for (let delay = 100; delay <= 2700; delay++) {
    const plan = createReactionTestCadencePlan(1000, delay, 9500);
    expect(plan.firstRedAt).toBe(9500 + delay);
    expect(plan.cues.map(cue => cue.at - plan.firstRedAt)).toEqual([0,120,240,360]);
  }
});
