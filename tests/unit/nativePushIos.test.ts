...
describe('native iOS push integration', () => {
  ...
  it('enables app push, preserves Watch capability scope, and ships build 15', () => {
    expect(entitlements).toContain('<key>aps-environment</key>');
    expect(project).toContain('com.apple.Push');
    expect(project.match(/CURRENT_PROJECT_VERSION = 15;/gu)).toHaveLength(4);
    expect(project).not.toContain('CURRENT_PROJECT_VERSION = 14;');
