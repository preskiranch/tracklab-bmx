...
async function mockReactionAccount(page: Page, options: {
  ...
  const state = {
    averageBestMs: options.averageBestMs ?? null,
...
test('completed group average remains visible when slower than the existing PR', async ({ page }, testInfo) => {
  const mock = await mockReactionAccount(page, { personalBestMs: 25, averageBestMs: 50 });
  await preparePredictableCadence(page);
  ...
  await expect(average).toBeInViewport();
  await expect.poll(() => view.evaluate(el => {
    const card=el.querySelector('.reaction-result-stack')!.getBoundingClientRect();
    const title=el.querySelector('.reaction-title')!.getBoundingClientRect();
    return card.top >= title.bottom + 4;
  })).toBe(true);
  await page.screenshot({path:testInfo.outputPath('group-average-landscape.png')});
  await view.getByRole('button',{name:'Try Again',exact:true}).click();
