...
export function PairingRail({ players, samplesByDevice, onAssign, onAutoAssign }: PairingRailProps) {
  ...
  return (
    <aside className="pairing-rail" aria-label="Bike pairing">
      <div className="rail-heading">
        <div>
          <h2>Bike Pairing</h2>
          <p>{Math.min(4, devices.length)} detected / max 4</p>
        </div>
        <button className="square-button" type="button" onClick={onAutoAssign} aria-label="Auto assign bikes">
          <Link size={18} />
        </button>
      </div>
      ...
      <div className="pairing-list">
        {players.length === 0 && (
          <div className="empty-panel">
            Pedal a Wattbike for a few seconds so the ANT+ bridge can detect it.
          </div>
        )}
