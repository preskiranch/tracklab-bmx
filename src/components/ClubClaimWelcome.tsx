...
export default function ClubClaimWelcome({ onChoose, onClose }: { onChoose: (role: 'athlete' | 'parent') => void; onClose: () => void }) {
  return <div className="child-setup-overlay"><section className="family-card child-setup-dialog" role="dialog" aria-modal="true" aria-labelledby="club-claim-welcome">
    ...
    <p>Adults claim their own profile. Parents create or sign into their own account first, claim the child’s studio record, then sign in on the child’s phone and assign that child from Family. Existing records stay connected.</p>
    <p>Also training yourself? Claim your own invite as the athlete first. Then open each child’s invite as the parent using the same account.</p>
