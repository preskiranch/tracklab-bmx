...
.utility-fullscreen .get-pulled-view{box-sizing:border-box;width:100%;height:100%;min-height:0;overflow:hidden;grid-template-rows:minmax(0,1fr) auto;grid-auto-rows:auto;gap:clamp(6px,1.2vh,12px);gap:clamp(6px,1.2dvh,12px);padding:max(10px,calc(env(safe-area-inset-top,0px) + 10px)) max(10px,calc(env(safe-area-inset-right,0px) + 10px)) max(10px,calc(env(safe-area-inset-bottom,0px) + 10px)) max(10px,calc(env(safe-area-inset-left,0px) + 10px));background:var(--panel);color:var(--ink)}
...
.get-pulled-exit-fullscreen,.get-pulled-end-session-fullscreen{position:static;display:inline-flex;align-items:center;justify-content:flex-start;gap:7px;box-sizing:border-box;width:100%;min-height:44px;padding:0 14px;border:1px solid rgba(255,255,255,.3);border-radius:10px;background:rgba(0,0,0,.9);color:#fff;font:inherit;font-weight:800;box-shadow:0 8px 24px rgba(0,0,0,.3)}
