    ...
    articles: [
      {
        ...
        paragraphs: [
          ...
          'Your card shows Average PR (your best average of three consecutive valid attempts) and Single best (your fastest individual attempt). After every completed group, This group’s average shows that group’s result and the difference from Average PR, even when it is slower. Only Average PR ranks on the leaderboard. A false start resets the unfinished group to zero; completing three attempts starts a new group. Leaving Reaction Test starts a fresh group. Previous single-attempt leaderboard scores do not qualify. Times are measured from the first red tone and displayed to thousandths of a second. Use Hide my time or Show my time to control your entry. Free and paid accounts are eligible.',
        ],
      },
      {
        title: 'BMX Race Intervals',
        paragraphs: [
          'Choose Ready to race in BMX Race Intervals to see the currently playable mapped courses. All tracks · Request mapping keeps the complete directory available: filter by Country and State / Region, optionally search by track name, then choose Request mapping for an unmapped track. A checkmark confirms the request and the team receives an email. Requests are saved to your account; a future release date is not guaranteed.',
